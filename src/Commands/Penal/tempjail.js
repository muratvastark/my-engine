...
exports.run = async (Moderation, message, args) => {
    ...
    const NewPenal = await Penal.addPenal(Member.id, message.author.id, "JAIL", Reason, false, DateNow, FinishDate, Member.roles ? Member.roles.cache.map((x) => x.id) : null);
    ...
    message.channel.send(Embed.setColor("RANDOM").setDescription((Member.roles ? `${Member} üyesi` : `**${Member.tag}** (\`${Member.id}\`) kullanıcısı sunucuda olmamasına rağmen `) + ` ${TimeString} boyunca cezalıya atıldı. (\`${NewPenal.Id}\`)`));

    const Channel = message.guild.channels.cache.get(Jail.Channel);
    if (Channel) Channel.send(Embed.setDescription((Member.roles ? `${Member} (\`${Member.id}\`) üyesi` : `**${Member.tag}** (\`${Member.id}\`) kullanıcısı sunucuda olmamasına rağmen`) + ` ${TimeString} süreliğine ${message.author} tarafından cezalıya atıldı. Sebep: \`${Reason}\``).setColor("RED"))
};
