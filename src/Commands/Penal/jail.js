...
exports.run = async (Moderation, message, args) => {
    ...
    message.channel.send(Embed.setColor("RANDOM").setDescription((Member.roles ? `${Member} (\`${Member.id}\`) üyesi` : `**${Member.tag}** (\`${Member.id}\`) kullanıcısı sunucuda olmamasına rağmen`) + ` cezalıya atıldı. (\`#${NewPenal.Id}\`)`));

    const Channel = message.guild.channels.cache.get(Jail.Channel);
    if (Channel) Channel.send(Embed.setDescription((Member.roles ? `${Member} (\`${Member.id}\`) üyesi` : `**${Member.tag}** (\`${Member.id}\`) kullanıcısı sunucuda olmamasına rağmen`) + ` ${message.author} tarafından cezalıya atıldı. Sebep: \`${Reason}\``).setColor("RED"));
};
